  ...
  private void loadQuizItems(long quizSetId, boolean isResume) {
    ...
    if (isResume) {
      ...
      if (progress != null) {
        ...
        try {
          quizIds = gson.fromJson(progress.quizIds, new TypeToken<List<Long>>() {
          }.getType());
          userAnswers = gson.fromJson(progress.userAnswers, new TypeToken<Map<Long, List<Integer>>>() {
          }.getType());
          results = gson.fromJson(progress.sessionResults, new TypeToken<List<QuizAttemptResult>>() {
          }.getType());
        } catch (Exception e) {
          loadQuizItems(quizSetId, false);
          return;
